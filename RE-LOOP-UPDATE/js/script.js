...
function getPagePath(pageName) {
    ...
    if (pageName === 'index.html') return isPages ? '../index.html' : 'index.html';
    return isPages ? pageName : 'pages/' + pageName;
}
function getAssetPath(assetName) {
    if (!assetName) return assetName;
    const isPages = window.location.pathname.includes('/pages/');
    if (assetName.startsWith('assets/')) {
        return isPages ? '../' + assetName : assetName;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function performSearch(query) {
        ...
        window.location.href = getPagePath('shop.html') + `?search=${term}`;
    }
    ...
    if (viewCampaignBtn) {
        viewCampaignBtn.addEventListener('click', () => {
            ...
            alert('âœ¨ Item kampanye RE-LOOP berhasil ditambahkan ke keranjang belanja Anda!');
        });
    }
    ...
    if (topProductsTrack && topDashItems.length > 0) {
        ...
        topProductCards.forEach(card => {
            ...
            card.addEventListener('click', (e) => {
                ...
                let productId = card.getAttribute('data-id') || '1';
                const name = card.querySelector('.top-card-name')?.textContent || '';
                const img = card.querySelector('img')?.src || '';
                const price = card.querySelector('.top-card-price')?.textContent || '';
                if (name) {
                    productId = 'dyn_' + name.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
                    sessionStorage.setItem('tempProd_' + productId, JSON.stringify({ name, img, price }));
                }
                window.location.href = getPagePath('product-detail.html') + `?id=${encodeURIComponent(productId)}`;
            });
        });
    }
    ...
    bestsellerCards.forEach(card => {
        ...
        if (miniCartBtn) {
            miniCartBtn.addEventListener('click', (e) => {
                ...
                const titleEl = card.querySelector('.bestseller-title');
                const priceEl = card.querySelector('.price-current');
                const imgEl = card.querySelector('.product-img');
                const productName = titleEl ? titleEl.textContent.trim() : 'Produk RE-LOOP';
                const priceRaw = priceEl ? priceEl.textContent : '$0';
                // convert price to IDR number
                const priceNum = priceRaw.includes('Rp') 
                    ? parseInt(priceRaw.replace(/[^0-9]/g,'')) 
                    : Math.round(parseFloat(priceRaw.replace(/[^0-9.]/g,'')) * 15000);

                addToCart({
                    id: productName.replace(/\s+/g, '-').toLowerCase(),
                    name: productName,
                    price: priceNum || 50000,
                    image: imgEl ? imgEl.src : getAssetPath('assets/logo.png'),
                    qty: 1
                });
        ...
        card.addEventListener('click', (e) => {
            ...
            let productId = btn ? btn.getAttribute('data-id') : '1';
            const name = card.querySelector('.bestseller-title, .bestseller-name, h3')?.textContent || '';
            const img = card.querySelector('img')?.src || '';
            const price = card.querySelector('.bestseller-price, .price')?.textContent || '';
            if (name) {
                productId = 'dyn_' + name.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
                sessionStorage.setItem('tempProd_' + productId, JSON.stringify({ name, img, price }));
            }
    ...
    // 6b. Product Detail Page (PDP), Marketplace Comparison, Login Page & Cart Page Logic
    ...
    if (accountSettingsForm) {
        accountSettingsForm.addEventListener('submit', (e) => {
            e.preventDefault();
            alert('âœ¨ Pengaturan akun Anda ("Christopher Davis") telah berhasil diperbarui!');
        });
    }
});
...
function requireAuth(featureName = 'fitur ini', redirectUrl = null) {
    ...
    const proceed = confirm(`ðŸ”’ Akses Terbatas:\n\nSilakan login terlebih dahulu untuk menggunakan ${featureName}.\n\nBuka halaman login sekarang?`);
    if (proceed) {
        window.location.href = getPagePath('login.html') + `?redirect=${encodeURIComponent(currentUrl)}`;
    }
    return false;
}
...
function syncNavbarAuthState() {
    ...
    const cartTotal = cartItems.reduce((s, i) => s + (i.qty || 1), 0);
    cartBadges.forEach(b => b.textContent = cartTotal);

    if (userProfileBtn) {
        if (loggedIn) {
            userProfileBtn.href = getPagePath('profile.html');
            ...
            userProfileBtn.href = getPagePath('login.html');
        ...
        logoutLinks.forEach(l => {
            if (l.textContent.includes('Log Out') || l.innerHTML.includes('ri-logout-box-r-line')) {
                l.addEventListener('click', (e) => {
                    ...
                    alert('ðŸ‘‹ Anda telah berhasil keluar dari akun.');
                    window.location.href = getPagePath('login.html');
                });
            }
        });
    ...
    if (wishlistHeaderBtn) {
        wishlistHeaderBtn.addEventListener('click', (e) => {
            if (!requireAuth('daftar Wishlist Anda')) {
                ...
                alert('âœ¨ Membuka daftar wishlist Anda...');
            }
        });
    }
...
function showGlobalCartToast(productName) {
    ...
    if (!toast) {
        ...
        toast.innerHTML = `<i class="ri-shopping-cart-2-fill" style="color:#EA580C;font-size:1.1rem;"></i>
            ...
            <a href="${getPagePath('cart.html')}" style="color:#EA580C;font-weight:700;margin-left:8px;text-decoration:none;">Lihat</a>`;
...
function initCartPage() {
    ...
    function renderCart() {
        ...
        const headerActions = document.querySelector('.cart-header-actions');
        ...
        if (cart.length === 0) {
            ...
            if (cartItemCountLabel) cartItemCountLabel.textContent = '0 item';
            return;
        }
            ...
            cartList.insertAdjacentHTML('beforeend', `
                <div class="cart-item-card">
                    <div class="cart-item-checkbox">
                        <input type="checkbox" class="item-select-check" checked>
                    </div>
                    <div class="cart-item-image">
                        <img src="${getAssetPath(item.image) || getAssetPath('assets/logo.png')}" alt="${item.name || 'Produk'}" style="width:80px;height:80px;object-fit:cover;border-radius:10px;">
                    </div>
                    <div class="cart-item-details">
                        <h3 class="cart-item-title">${item.name || 'Produk RE-LOOP'}</h3>
                        <div class="cart-item-store">RE-LOOP Recommended</div>
                        <div class="cart-item-price">${formatRp(itemPrice)}</div>
                    </div>
                    <div class="cart-item-actions">
                        <button class="cart-item-del-btn" data-index="${index}" title="Hapus"><i class="ri-delete-bin-line"></i></button>
                        <div class="cart-qty-control">
                            <button class="qty-btn qty-minus" data-index="${index}"><i class="ri-subtract-line"></i></button>
                            <span class="qty-display">${itemQty}</span>
                            <button class="qty-btn qty-plus" data-index="${index}"><i class="ri-add-line"></i></button>
                        </div>
                    </div>
                </div>
            `);
        ...
        if (cartItemCountLabel) cartItemCountLabel.textContent = `${totalItems} item dipilih`;
        if (summarySubtotal) summarySubtotal.textContent = 'Rp ' + subtotal.toLocaleString('id-ID');
        if (summaryTotal) summaryTotal.textContent = 'Rp ' + (subtotal + shipping).toLocaleString('id-ID');
        ...
        cartList.querySelectorAll('.qty-plus').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (c[idx]) { c[idx].qty = (c[idx].qty || 1) + 1; saveCartItems(c); renderCart(); }
            });
        });
        cartList.querySelectorAll('.qty-minus').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (c[idx]) {
                    ...
                    saveCartItems(c); renderCart();
                }
            });
        });
        cartList.querySelectorAll('.cart-item-del-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const idx = parseInt(e.currentTarget.getAttribute('data-index'));
                const c = getCartItems();
                c.splice(idx, 1);
    ...
    const cart = getCartItems();
    const total = cart.reduce((s, i) => s + (i.qty || 1), 0);
    document.querySelectorAll('.cart-count').forEach(b => b.textContent = total);

    // Checkout button logic has been moved directly to cart.html
    // so it properly redirects to checkout.html without clearing the cart prematurely.
    // Checkout Modal OK Button
    const checkoutOkBtn = document.getElementById('checkoutOkBtn');
    if (checkoutOkBtn) {
        checkoutOkBtn.addEventListener('click', () => {
    ...
    if (applyPromoBtn && promoInput) {
        applyPromoBtn.addEventListener('click', () => {
            ...
            if (code === 'RELOOP10') {
                if (promoStatus) { promoStatus.textContent = 'âœ… Kode promo RELOOP10 berhasil! Diskon 10% diterapkan.'; promoStatus.style.color = '#10B981'; }
            } else if (code === '') {
                if (promoStatus) { promoStatus.textContent = 'Masukkan kode promo terlebih dahulu.'; promoStatus.style.color = '#EF4444'; }
            } else {
                if (promoStatus) { promoStatus.textContent = `âŒ Kode "${code}" tidak valid atau sudah kadaluarsa.`; promoStatus.style.color = '#EF4444'; }
            }
        });
    }
    ...
    if (deleteSelectedBtn) {
        deleteSelectedBtn.addEventListener('click', () => {
            const checks = cartList.querySelectorAll('.item-select-check:checked');
            const indices = Array.from(checks).map(ch => {
                const card = ch.closest('.cart-item-card');
                const delBtn = card ? card.querySelector('.cart-item-del-btn') : null;
                return delBtn ? parseInt(delBtn.getAttribute('data-index')) : -1;
            }).filter(i => i >= 0).sort((a, b) => b - a); // sort descending to splice correctly

            const c = getCartItems();
            indices.forEach(i => c.splice(i, 1));
            saveCartItems(c);
            renderCart();
        });
    }
...
function buildNotifDropdown() {
    ...
    drop.innerHTML = `
        ...
        <div class="notif-footer">
            <a href="#" class="notif-see-all-link">Lihat semua notifikasi â†’</a>
        </div>
    `;
...
function initProductDetailPage() {
    ...
    let product = productDatabase[productId];
    let stored = sessionStorage.getItem('tempProd_' + productId);
    let basicProd = null;
    if (stored) {
        basicProd = JSON.parse(stored);
        basicProd.image = basicProd.img || basicProd.image;
    } else if (window.reloopProducts) {
        basicProd = window.reloopProducts.find(p => p.id === productId);
    }
    
    if (!product && basicProd) {
        if (basicProd) {
            product = {
                title: basicProd.name,
                category: basicProd.category || 'Kategori Pilihan',
                brand: 'Pilihan Editor',
                crumb: basicProd.name + ' detail',
                variant: 'Standard Edition',
                rating: basicProd.rating || '4.8',
                reviews: (basicProd.reviews || 'Ratusan') + ' ulasan pembeli',
                aiScore: '92',
                aiVerdict: 'Sangat Direkomendasikan',
                aiSummary: 'Skor AI agregat dari ulasan di berbagai marketplace populer.',
                bestPrice: basicProd.price,
                bestSource: 'di Marketplace Pilihan',
                avgPrice: basicProd.price,
                savings: basicProd.discount ? 'Hemat ' + basicProd.discount : 'Harga Terbaik',
                aiBestStore: 'Official Store',
                images: [ getAssetPath(basicProd.image) ],
                pros: ['Kualitas barang sesuai deskripsi.', 'Banyak direkomendasikan pembeli.', 'Pengiriman cepat.'],
                cons: ['Stok terbatas di beberapa wilayah.', 'Varian warna cepat habis.'],
                buyTiming: 'Berdasarkan tren harga, sekarang adalah waktu yang tepat untuk membeli.',
                sentimentPercent: '92%',
                sentimentStatus: 'Sangat Positif',
                sentimentDetail: '92% dari ulasan menyatakan kepuasan yang tinggi.',
                posPercent: '92%',
                negPercent: '8%',
                aspects: [
                    { name: 'Kualitas', val: '95%' },
                    { name: 'Harga', val: '90%' },
                    { name: 'Kenyamanan', val: '88%' }
                ],
                stores: [
                    {
                        id: 'tokopedia',
                        name: 'Tokopedia - Official Partner',
                        marketplace: 'Tokopedia',
                        badgeClass: 'btn-tokopedia',
                        badgeColor: '#42B549',
                        isOfficial: true,
                        ...
                        sales: '1k+ terjual',
                        city: 'Jakarta',
                        price: basicProd.price,
                        priceNum: parseInt(basicProd.price.replace(/[^\d]/g, '')) || 0,
                        oldPrice: '',
                        discount: basicProd.discount || '',
                        diffNote: '?? Rekomendasi Teratas',
                        isBestDeal: true,
                        hasFreeShipping: true,
                        hasCashback: true,
                        hasInstallment: false,
                        ...
                        shipping: 'Reguler / Instant',
                        payment: 'Transfer Bank / E-Wallet',
                        link: '#'
                    }
                ]
            };
    ...
    function handleAddToCart(isDirectBuy = false) {
        ...
        const priceRaw = product.bestPrice || 'Rp 0';
        ...
        if (isDirectBuy) {
            ...
            addToCart({
                ...
                name: product.title,
                price: priceNum,
                image: mainImg ? mainImg.src : (product.images?.[0] ? getAssetPath(product.images[0]) : getAssetPath('assets/logo.png')),
                qty: qty
            });
...
function initComparePage() {
    ...
    if (backLink) {
        backLink.href = getPagePath('product-detail.html') + `?id=${encodeURIComponent(productId)}`;
    }
                ...
                tr.innerHTML = `
                    <td>
                        <div class="table-store-cell">
                            ...
                            <div>
                                <div>${store.name}</div>
                                <div style="font-size: 0.75rem; color: #64748B;">${store.officialLabel} â€¢ ${store.city}</div>
                            </div>
                        </div>
                    </td>
                    <td>
...
function initLoginPage() {
        ...
        loginForm.addEventListener('submit', (e) => {
            ...
            setTimeout(() => {
                ...
                if (redirectParam && (redirectParam.includes('.html') || redirectParam.startsWith('http'))) {
                    // Came from a protected feature â€” show onboarding first then continue
                    if (!onboardingDone) {
                        window.location.href = getPagePath('onboarding.html') + `?redirect=${encodeURIComponent(redirectParam)}`;
                    } else {
                        window.location.href = redirectParam;
                    }
                } else {
                    // Normal login â€” show onboarding then go home
                    if (!onboardingDone) {
                        window.location.href = getPagePath('onboarding.html');
                    } else {
                        window.location.href = getPagePath('index.html');
                    }
                }
    ...
    if (forgotPassLink) {
        forgotPassLink.addEventListener('click', (e) => {
            ...
            if (email) {
                alert(`âœ¨ Tautan pemulihan kata sandi telah dikirimkan ke "${email}". Silakan periksa inbox/SMS Anda.`);
            }
        });
    }
...
// Automatically set active class on nav links based on current URL
document.addEventListener('DOMContentLoaded', () => {
    const currentPath = window.location.pathname.split('/').pop() || 'index.html';
    const navLinks = document.querySelectorAll('.nav-menu .nav-link');
    navLinks.forEach(link => {
        const linkPath = link.getAttribute('href').split('#')[0];
        if (link.getAttribute('href') === currentPath || linkPath === currentPath && !link.getAttribute('href').includes('#')) {
            link.classList.add('active');
        } else {
...
document.addEventListener('DOMContentLoaded', () => {
    const searchInputs = document.querySelectorAll('input[type="text"][placeholder*="Search"], input[type="text"][placeholder*="Ketik nama"], #quickSearchInput, #heroSearchInput');
    
    // Helper to ensure products.js is loaded
    const getProducts = () => {
        return new Promise((resolve) => {
            if (window.reloopProducts) {
                resolve(window.reloopProducts);
            } else {
                const script = document.createElement('script');
                script.src = getAssetPath('js/products.js');
                script.onload = () => resolve(window.reloopProducts);
    ...
    searchInputs.forEach(input => {
        ...
        input.addEventListener('input', async (e) => {
            ...
            // Prioritize items that start with the query, then include items that contain the query
            let matches = products.filter(p => p.name.toLowerCase().startsWith(query));
            if (matches.length < 3) {
                const includesMatches = products.filter(p => !p.name.toLowerCase().startsWith(query) && p.name.toLowerCase().includes(query));
                matches = [...matches, ...includesMatches];
            }
            
            // Slice top 3 as requested
            matches = matches.slice(0, 3);
            
            if (matches.length > 0) {
                dropdown.innerHTML = matches.map(p => `
                    <div class="autocomplete-item" onclick="window.location.href=getPagePath('product-detail.html')">
                        <img src="${getAssetPath(p.image)}" class="autocomplete-item-img" alt="${p.name}">
                        <div class="autocomplete-item-info">
                            <div class="autocomplete-item-title">${p.name}</div>
                ...
                dropdown.innerHTML = '<div style="padding: 12px; font-size: 0.8rem; color: #64748B;">No products found</div>';
                dropdown.classList.add('active');
            }
        });
